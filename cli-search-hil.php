...
// Forked from /media/www/html/croogle/cli-search.php
// This should output more information (ie. hil-node number and not the query but
...
//
// Must be able to 'easily' run a command from PHP and get the output with 
// piping support.
...
if (!empty($ids)) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    foreach ($ids as $c => $id) {
      ...
      $fullpath = $row['text'];
      $shortpath = $row['minimised'];

      $paths[] = $fullpath;

      $nodenumber = trim(runcommand("/var/www/croogle/get-node-number.sh", $fullpath, '/var/www/croogle'));
      $nodes[] = $nodenumber;

      $matchdata[] = $id.','.$nodenumber.','.$fullpath;
    }
  }
}

#print $index.':'.implode(' ', $ids).':'.getLongestWord($q);
print $index.':'.implode(' ', $matchdata).':'.$q;
